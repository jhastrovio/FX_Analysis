#!/usr/bin/env python3
"""
Analytics-only utility: Check model count from read-only data sources.

This script reads from the FX data estate (read-only) to verify model availability.
It does not modify data or produce datasets.
...
def count_from_master_matrix(processed_path):
    """Count models from consolidated return matrix (temporary analysis artifact).
    
    Note: This reads from ephemeral analysis outputs, not authoritative datasets.
    """
    master_files = list(processed_path.glob("*Master*.csv"))
    if not master_files:
        return None
    
    # Use the most recent consolidated matrix (temporary analysis artifact)
...
def main():
    ...
    # 3. Count from consolidated matrix (temporary analysis artifact)
    print("\n3️⃣  Checking Consolidated Return Matrix (ephemeral)...")
    matrix_count = count_from_master_matrix(processed_path)
    if matrix_count:
        count, filename = matrix_count
        results['matrix'] = count
        print(f"   ✅ Consolidated matrix '{filename}' contains {count} models (ephemeral)")
    else:
        print(f"   ⚠️  Consolidated matrix not found (may need to run consolidation first)")
        results['matrix'] = None
    ...
    if results.get('matrix'):
        print(f"   Consolidated matrix columns: {results['matrix']} (ephemeral)")
    ...
    if counts:
        ...
        print("   Try running: python -m lib.data_consolidate --verbose")
